...
(
    ...
    CONSTRAINT FK_EMPLEADO_HACIENDA FOREIGN KEY(idhacienda) REFERENCES HACIENDAS(id),
    CONSTRAINT FK_EMPLEADO_LABOR FOREIGN KEY(idlabor) REFERENCES HAC_LABORES(id)
)
alter table HAC_EMPLEADOS add codigo varchar(20) default null
drop table HAC_EMPLEADOS

select * from HAC_EMPLEADOS
...
(
    ...
    CONSTRAINT FK_BODEGA_HACIENDA FOREIGN KEY(idhacienda) REFERENCES HACIENDAS(id)
)
...
drop table BOD_MATERIALES
select * from BOD_MATERIALES order by updated_at desc

declare @date datetime = '2020-05-04'
...
alter table bod_egresos drop column idlabor varchar(15)
...
(
    ...
    movimiento   varchar(50) default 'EGRE-ART',
...
alter table BOD_DET_EGRESOS add id_origen int default null 

select * from HAC_EMPLEADOS
drop table BOD_EGRESOS
select * from BOD_EGRESOS
select * from BOD_DET_EGRESOS
select * from HAC_INVENTARIO_EMPLEADO
select * from BOD_MATERIALES
delete BOD_EGRESOS
delete BOD_DET_EGRESOS
delete HAC_INVENTARIO_EMPLEADO

delete from BOD_DET_EGRESOS where id between 40 and 42

update BOD_DET_EGRESOS set fecha_salida = '2020-05-05'

update HAC_INVENTARIO_EMPLEADO set tot_egreso = 150, sld_final = 150 where idmaterial = 1
update HAC_INVENTARIO_EMPLEADO set tot_egreso = 120, sld_final = 120  where idmaterial = 2
update HAC_INVENTARIO_EMPLEADO set tot_egreso = 1, sld_final = 1  where idmaterial = 6

update BOD_MATERIALES set stock = 4102.00 where id = 2

/*BITACORA DE MOVIMIENTOS DE MATERIALES
*/
CREATE TABLE HAC_INVENTARIO_EMPLEADO
(
    id         int identity (1,1) not null,
    ...
    updated_at datetime default null,
    estado     bit      default 1,
    CONSTRAINT PK_INV_EMPLEADO PRIMARY KEY (id),
...
CREATE TABLE SIS_CALENDARIO_DOLE
(
